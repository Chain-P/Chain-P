...
def simple_request(func_name, query, variables):
    ...
    if request.status_code == 200:
        body = request.json()
        if body.get('data') is None or body.get('errors'):
            raise Exception(func_name, ' returned a GraphQL error:', body.get('errors'), QUERY_COUNT)
        return request
...
def graph_commits(start_date, end_date):
    """
    Uses GitHub's GraphQL v4 API to return total commit count in a date range
    """
...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    ...
    only counts commits authored by me
    """
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    """
    Uses GitHub's GraphQL v4 API to query all repositories accessible to this token
...
def force_close_file(data, cache_comment):
    """
    Forces the cache file to close, preserving whatever data was written before a crash
    """
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    with open(filename, 'w') as f:
        f.writelines(cache_comment)
...
def commit_counter(comment_size):
    """
    Counts total commits using the cache file created by cache_builder.
    """
    total_commits = 0
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    with open(filename, 'r') as f:
